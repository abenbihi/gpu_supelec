...
{
    if [ $mode == 0 ]; then
	#ssh $login@term2.grid "oarsub -r \"$(date +'%F %T')\" -p \"network_address in ('sh01', 'sh02', 'sh03', 'sh04','sh05', 'sh06', 'sh07', 'sh08', 'sh09', 'sh10', 'sh11', 'sh12','sh13','sh14','sh15','sh16')\" -l nodes=1,walltime=1000:00:00" > reservation$job_num.log
  # sh01 has cache issues
	ssh $login@term2.grid "oarsub -r \"$(date +'%F %T')\" -p \"network_address in ( 'sh02', 'sh03', 'sh04','sh05', 'sh06', 'sh07', 'sh08', 'sh09', 'sh10', 'sh11', 'sh12','sh13','sh14','sh15','sh16')\" -l nodes=1,walltime=1000:00:00" > reservation$job_num.log
    else
